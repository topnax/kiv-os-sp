...
\begin{itemize}
    \item \texttt{kiv\_os::THandle handle} - představuje identifikátor (\textbf{PID}\footnote{process identifier}) procesu
    \item \texttt{kiv\_os::THandle std\_in} - představuje identifikátor standardního vstupu procesu
    \item \texttt{kiv\_os::THandle std\_out} - představuje identifikátor standardního výstupu procesu
    \item \texttt{char *program\_name} - představuje název programu, který proces vykonává
    \item \texttt{std::filesystem::path working\_directory} - představuje název programu, který proces vykonává
\end{itemize}
